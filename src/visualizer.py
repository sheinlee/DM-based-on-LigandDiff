...
def save_xyz_file(path, one_hot, positions, names,batch_seg):
    idx2atom = const.IDX2ATOM
    unique_indices = torch.unique(batch_seg)
    for idx in unique_indices:
        pos = positions[batch_seg == idx]
        onehot = one_hot[batch_seg == idx]
        n_atoms = torch.sum(batch_seg == idx).item()
        f = open(os.path.join(path, f'{names[idx]}.xyz'), "w")
        f.write("%d\n\n" % n_atoms)
        atoms = torch.argmax(onehot, dim=1)
        for atom_i in range(len(pos)):
            atom = idx2atom[atoms[atom_i].item()]
            f.write("%s %.9f %.9f %.9f\n" % (
                atom, pos[atom_i, 0], pos[atom_i, 1], pos[atom_i, 2]
            ))
        f.close()
...
def load_xyz_files(path):
    ...
    for fname in os.listdir(path):
        if fname.endswith(f'.xyz'):
            files.append(fname)
    files = sorted(files, key=lambda f: -int(f.replace(f'.xyz', '').split('_')[-1]))
...
def visualize_chain(
        path, spheres_3d=False, bg="black", alpha=1.0, wandb=None, mode="chain", fragment_mask=None
):
